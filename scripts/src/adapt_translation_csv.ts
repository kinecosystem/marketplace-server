...
function writeCsvDataToFile(data: any[], fileName: string, resolve: (value?: any) => void, reject: (reason?: any) => void) {
	...
	writeFile(fileName, csvExporter.generateCsv(data, true), err => {
		...
		console.log("Adapted CSV saved as", fileName);
		resolve(fileName);
	});
}
...
export async function processFile(translationFile: string, fileToTranslate: string, saveAs: string | null = null, rowOffSet: number = 1) {
	/***
	 * Takes a template (CSV) file (fileToTranslate) generated by admin/translations.writeCsvTemplateToFile and a translation file (translationFile)
	 * and adapts the translations to the offers in the template, Saves the to the file specified with SaveAs
	***/
